...
from src.color_pipeline.localize import HeuristicMaskProvider
...
def test_bicolor_returns_stable_proportions(tmp_path):
    ...
    assert result.primary.matched_name == "Crimson"
    assert len(result.dominant_colors) >= 1
    assert abs(result.dominant_colors[0].proportion - 0.7) < 0.35
