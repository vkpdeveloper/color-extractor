...
class QueryProcessor:
    def __init__(self, model="en_core_web_sm"):
        try:
            ...
            # Fallback path for environments where the packaged model is absent.
            try:
    ...
    def _add_fashion_patterns(self):
        # Add EntityRuler before NER when available; otherwise attach at pipeline start.
        if "entity_ruler" not in self.nlp.pipe_names:
            if "ner" in self.nlp.pipe_names:
                ruler = self.nlp.add_pipe("entity_ruler", before="ner")
            else:
                ruler = self.nlp.add_pipe("entity_ruler", first=True)
        else:
